...
class PicPlayAction extends CommonAction {
	...
	public function index() {
		...
		$list = M('picplay')->order('sort')->limit($limit)->select();
	...
	public function addHandle() {
		...
		if($id = M('picplay')->add($data)) {
			...
			$this->success('添加成功',U(GROUP_NAME. '/PicPlay/index'));
		}else {
			$this->error('添加失败');
		}
	}
	...
	public function editHandle() {
		...
		if (false !== M('picplay')->save($_POST)) {
			...
			$this->success('修改成功', U(GROUP_NAME. '/PicPlay/index', array('pid' => $pid)));
		}else {
	...
	public function del() {
		...
		if (M('picplay')->delete($id)) {			
			M('attachmentindex')->where(array('arcid' => $id, 'modelid' => 0, 'desc' => 'link'))->delete();
			$this->success('彻底删除成功', U(GROUP_NAME. '/PicPlay/index'));
		}else {
			$this->error('彻底删除失败');
		}
	}
	...
	public function delBatch() {
		...
		if (M('picplay')->where($where)->delete()) {
